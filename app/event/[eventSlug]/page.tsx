...
export default function EventDetailPage() {
    ...
    return (
      ...
      >
        ...
        <section className="card decorated-card notification-card">
          ...
          <p>{eventConfig.name}の導線は開催期間中のみ表示されます。復刻時にまた遊べるようにする予定です。</p>
  ...
  const eventEggMonster = monsters.find((monster) => monster.monsterId === eventConfig.freeEggMonsterId);
  const eventEggName = eventEggMonster?.name ?? "イベントたまご";
  const featuredMonsterNames = eventMonsters
    .slice(0, 2)
  ...
  return (
    ...
    >
      ...
      <section className="card decorated-card event-progress-card">
        ...
        <div className="event-progress-note">
          {eventState?.hasCompletedLoginMission
            ...
            : `期間中に${eventConfig.mission.loginDaysRequired}日ログインすると、${eventConfig.name}の完走条件を達成できます。`}
        </div>
      </section>
      ...
      <section className="card decorated-card">
        ...
        <p className="event-progress-note">{featuredMonsterNames ? `${featuredMonsterNames} が出現中！` : "イベントモンスターが出現中！"}タスクを達成してイベントモンスターを育てよう！</p>
      ...
      <section className="card decorated-card notification-card">
        ...
        <p>{eventConfig.name}限定の背景やフレーム、イベントたまごは専用ショップにまとめています。</p>
        <div className="notification-card-actions">
