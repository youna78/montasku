  ...
  return (
    <main className="page-shell">
      ...
      <section className="card decorated-card notification-card">
        ...
        <p className="shop-note">このブラウザだけで、イベント期間とモンスターリニューアル表示の判定に使う日時を上書きします。</p>
        ...
        <div className="event-progress-grid">
          {GAME_EVENTS.map((eventConfig) => (
            <div className="event-progress-item" key={eventConfig.eventId}>
              ...
              <strong>{getEventStatusLabel(eventConfig, currentNow)} / {getRemainingDaysLabel(eventConfig, currentNow)}</strong>
            </div>
          ))}
        </div>
