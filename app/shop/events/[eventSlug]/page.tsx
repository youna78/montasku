...
export default function EventShopDetailPage() {
  ...
  const onClaimFreeEgg = () => {
    ...
    const eggName = monsters.find((monster) => monster.monsterId === eventConfig.freeEggMonsterId)?.name ?? "イベントたまご";
    setMessage(`${eggName}を受け取りました`);
  };
  ...
  const onForceStartEgg = () => {
    ...
    if (!result.started) {
      if (result.reason === "no_egg") setMessage(`${eggName}を持っていません`);
      else if (result.reason === "already_active") setMessage(`すでに${eggName}を育成中です`);
      else setMessage(`${eggName}に切り替えできませんでした`);
      return;
    }
    setMessage(`いまのモンスターとお別れして、${eggName}に切り替えました`);
  ...
  const showLegacySpringShopItems = eventConfig.eventId === "spring_easter_2026";
  const starterCheckoutItem = showLegacySpringShopItems
  ...
  return (
    ...
    >
      ...
      <section className="card decorated-card quest-heading-card">
        <p>{eventConfig.name}限定の背景やフレーム、イベントたまごを交換できます。イベントモンスターを育てるには、受け取ったたまごを「次のたまごに予約する」でセットしてください。</p>
      </section>
      ...
      {eventConfig.freeCoinShopItems.length > 0 ? (
        <section className="card decorated-card">
          <div className="notification-card-head">
      ...
      {showLegacySpringShopItems && SHOP_EVENT_DECORATIONS.length > 0 ? (
        <>
          <section className="card decorated-card">
            <div className="notification-card-head">
      ...
      {showLegacySpringShopItems && SHOP_EVENT_BUNDLES.length > 0 ? (
        <>
          <section className="card decorated-card">
            <div className="notification-card-head">
